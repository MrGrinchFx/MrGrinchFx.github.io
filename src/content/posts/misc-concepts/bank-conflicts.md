...
The first observation is that regardless of where static shared memory allocation is called, the static shared memory are allocated aggregately in a big chunk. There is no padding in between and allocated continuously. The static shared memory are always allocated before dynamic shared memory.

The second observation is that dynamic shared memory are allocated in multiple of 16 bytes. 
